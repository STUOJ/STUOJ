...
func (query *{{.StructName}}) GenerateGroupCountOptions() *option.GroupCountOptions {
	options := option.NewGroupCountOptions(&field.{{.EntityName}}GroupField{})
	query.applyFilter(options)
	options.GroupBy.SetField(query.GroupBy)
	return options
}
`
...
func processQueryContext(dir string, structName string) error {
	...
	fileName := "generated_" + strings.ToLower(structName) + ".go"
