...
func Submit(req request.JudgeReq, reqUser model.ReqUser) (int64, error) {
	languageQuery := querycontext2.LanguageQueryContext{}
	languageQuery.Id.Add(req.LanguageId)
	languageQuery.Field = *query2.LanguageAllField
	...
	if problemDomain.Status < entity2.ProblemPublic && reqUser.Role < entity2.RoleAdmin {
	...
	submissionDomain := submission.NewSubmission(
		...
		submission.WithStatus(entity2.JudgeIE), // 设置为IE状态,保证在错误导致中断的情况下显示为IE
	...
	testcaseQuery.Field = *query2.TestcaseAllField
	...
	for i, tc := range testcaseDomain {
		judgementDomain := judgement.NewJudgement(
			...
			judgement.WithStatus(entity2.JudgeIE), //设置为IE状态,保证在错误导致中断的情况下显示为IE
	...
	for i, v := range runnerResult.TestResult {
		if v.Status.Id == int64(entity2.JudgeAC) {
		...
		judgementDomain := judgement.NewJudgement(
			...
			judgement.WithStatus(entity2.JudgeStatus(v.Status.Id)),
		)
	...
	submissionDomain = submission.NewSubmission(
		...
		submission.WithStatus(entity2.JudgeStatus(runnerResult.Status.Id)),
