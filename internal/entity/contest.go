...
// Contest 比赛
type Contest struct {
	Id           uint64        `gorm:"primaryKey;autoIncrement;comment:比赛ID" json:"id"`
	UserId       uint64        `gorm:"not null;default:0;comment:用户ID" json:"-"`
	CollectionId uint64        `gorm:"not null;default:0;comment:题单ID" json:"-"`
	Status       ContestStatus `gorm:"not null;default:1;comment:状态" json:"status"`
	Format       ContestFormat `gorm:"not null;default:1;comment:赛制" json:"format"`
	TeamSize     uint8         `gorm:"not null;default:1;comment:组队人数" json:"team_size"`
	StartTime    time.Time     `gorm:"type:timestamp;not null;default:CURRENT_TIMESTAMP;comment:开始时间" json:"start_time"`
	EndTime      time.Time     `gorm:"type:timestamp;not null;default:CURRENT_TIMESTAMP;comment:结束时间" json:"end_time"`
